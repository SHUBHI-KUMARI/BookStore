...
export const BookCard = ({ 
  id, title, author, price, coverUrl, condition, conditionDetail, rating 
}: BookCardProps) => {
  return (
    <div className="group bg-white rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full ring-1 ring-black/5">
      ...
      <Link to={`/books/${id}`} className="relative h-64 w-full bg-gray-50 flex items-center justify-center p-6 overflow-hidden">
        {/* Badges */}
        <div className="absolute top-4 left-4 flex flex-col gap-2 z-10">
          <Badge variant={condition === 'NEW' ? 'warning' : 'primary'} size="md" className="uppercase tracking-wider">
            {condition}
          </Badge>
          {condition === 'USED' && conditionDetail && (
            <Badge variant="outline" size="sm" className="uppercase tracking-wider bg-white/90 backdrop-blur-sm shadow-sm border-gray-200">
              {conditionDetail}
            </Badge>
          )}
        </div>
      ...
      <div className="p-5 flex flex-col flex-grow">
        ...
        <div className="flex items-center gap-1 mb-3">
          ...
          <span className="text-xs text-gray-500 ml-1.5 font-medium">({(title.length * 3) + 12})</span>
        </div>

        {/* Title & Author */}
        <Link to={`/books/${id}`} className="group-hover:text-[var(--color-brand-muted-orange)] transition-colors inline-block">
        ...
        <div className="flex items-center justify-between border-t border-gray-100 pt-4 mt-2">
          ...
          <Button size="sm" variant="primary" className="rounded-full px-5 shadow-sm hover:shadow-md" aria-label={`Add ${title} to cart`}>
