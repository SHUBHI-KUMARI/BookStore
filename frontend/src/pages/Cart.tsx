      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {isCartEmpty ? (
          <div className="bg-white rounded-3xl p-12 text-center shadow-sm border border-black/5 flex flex-col items-center">
            ...
            <h2 className="text-2xl font-bold text-[var(--color-brand-dark-blue)] mb-3">
              Your cart is empty
            </h2>
            <p className="text-gray-500 mb-8 max-w-md mx-auto">
              Looks like you haven't added any books yet. Discover your next
              great read in our marketplace.
            </p>
            <Link to="/books">
          ...
          <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="flex-1 space-y-6">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-black/5 flex flex-col sm:flex-row items-center sm:items-start gap-6 transition-all hover:shadow-md"
                >
                  {/* Image */}
                  <Link
                    to={`/books/${item.bookId}`}
                    className="shrink-0 relative w-32 h-40 bg-gray-50 flex items-center justify-center rounded-xl overflow-hidden group"
                  >
                    <img
                      src={item.coverUrl}
                      alt={item.title}
                  ...
                  <div className="flex-1 flex flex-col items-center sm:items-start text-center sm:text-left w-full">
                    <div className="flex flex-col sm:flex-row sm:justify-between w-full gap-2 mb-2">
                      <div>
                        <Link
                          to={`/books/${item.bookId}`}
                          className="hover:text-[var(--color-brand-muted-orange)] transition-colors"
                        >
                          <h3 className="text-xl font-bold text-[var(--color-brand-dark-blue)]">
                            {item.title}
                          </h3>
                        </Link>
                        <p className="text-[var(--color-brand-brown)] text-sm">
                          {item.author}
                        </p>
                      </div>
                      <span className="text-xl font-bold text-[var(--color-brand-dark-blue)]">
            ...
            <div className="w-full lg:w-96 shrink-0">
              <div className="bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-black/5 sticky top-24">
                ...
                {/* Coupon Code */}
                <div className="mb-8">
                  <label className="text-xs font-bold text-[var(--color-brand-dark-blue)] mb-2 block uppercase tracking-wider">
                ...
                <div className="mt-6 text-center text-xs text-gray-500">
                  <p>
                    Secure checkout powered by ReBook. 30-day money-back
                    guarantee.
                  </p>
                </div>
              </div>
