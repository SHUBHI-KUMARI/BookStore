...
import { CreditCard, Wallet, MapPin, Truck, CheckCircle2, ShieldCheck, ChevronLeft } from 'lucide-react';
...
const ORDER_SUMMARY = {
  ...
  items: [
    ...
    { title: 'Project Hail Mary', qty: 1, price: 11.20 },
  ]
};
    ...
    return (
      <div className="bg-[var(--color-brand-cream)]/30 min-h-[80vh] flex items-center justify-center py-20 px-4">
        <div className="max-w-lg w-full bg-white p-10 rounded-3xl shadow-xl border border-black/5 text-center flex flex-col items-center">
          ...
          <h2 className="text-3xl font-serif font-black text-[var(--color-brand-dark-blue)] mb-3">Order Confirmed!</h2>
          <p className="text-[var(--color-brand-brown)] mb-2 font-medium">Order #RB-948210</p>
          <p className="text-gray-500 mb-8">
            Thank you for shopping with ReBook. We've sent a confirmation email with your order details and tracking information.
          </p>
    ...
    <div className="bg-gray-50 min-h-screen pb-24">
      ...
      <div className="bg-white border-b border-black/5 pt-8 pb-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
           <Link to="/cart" className="flex items-center text-sm font-bold text-[var(--color-brand-brown)] hover:text-[var(--color-brand-dark-blue)] transition-colors">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
            ...
            <form id="checkout-form" onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-black/5">
                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                  ...
                  <h2 className="text-xl font-bold text-[var(--color-brand-dark-blue)]">Shipping Details</h2>
                </div>
                ...
                <div className="mb-5">
                  <Input label="Street Address" required placeholder="123 Main St, Apt 4B" />
                </div>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                  <Input label="City" required placeholder="New York" className="md:col-span-1" />
                  <Select 
                  ...
                  />
                  <Input label="ZIP / Postal Code" required placeholder="10001" className="md:col-span-1" />
                </div>
              </div>
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-black/5">
                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                  ...
                  <h2 className="text-xl font-bold text-[var(--color-brand-dark-blue)]">Delivery Method</h2>
                </div>

                <div className="space-y-4">
                  <label className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-colors border-[var(--color-brand-muted-orange)] bg-[var(--color-brand-muted-orange)]/5`}>
                    <div className="flex items-center gap-3">
                      <input type="radio" name="delivery" defaultChecked className="w-4 h-4 text-[var(--color-brand-muted-orange)] border-gray-300 focus:ring-[var(--color-brand-muted-orange)]" />
                      <div>
                        <p className="font-bold text-[var(--color-brand-dark-blue)]">Standard Delivery</p>
                        <p className="text-sm text-gray-500">3-5 business days</p>
                      </div>
                    </div>
                    <span className="font-bold text-[var(--color-brand-dark-blue)]">$5.99</span>
                  </label>
                </div>
              </div>
              ...
              <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-black/5">
                <div className="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                  ...
                  <h2 className="text-xl font-bold text-[var(--color-brand-dark-blue)]">Payment Method</h2>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  >
                    <CreditCard className={`w-6 h-6 ${paymentMethod === 'credit-card' ? 'text-[var(--color-brand-dark-blue)]' : 'text-gray-400'}`} />
                    <span className={`text-sm font-bold ${paymentMethod === 'credit-card' ? 'text-[var(--color-brand-dark-blue)]' : 'text-gray-500'}`}>Credit Card</span>
                  </button>
                  <button 
                    type="button"
                  ...
                  >
                    ...
                    <span className={`text-sm font-bold ${paymentMethod === 'paypal' ? 'text-[#003087]' : 'text-gray-500'}`}>PayPal</span>
                  </button>
                </div>
                ...
                {paymentMethod === 'credit-card' && (
                  <div className="p-5 bg-gray-50 border border-gray-100 rounded-xl space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-5">
                      ...
                      <Input label="Security Code (CVV)" placeholder="123" required />
                    </div>
          ...
          <div className="w-full lg:w-[400px] shrink-0">
            <div className="bg-white rounded-3xl p-6 md:p-8 shadow-xl border border-black/5 sticky top-8">
              <h2 className="text-xl font-bold text-[var(--color-brand-dark-blue)] mb-6">Review Order</h2>
                ...
                {ORDER_SUMMARY.items.map((item, idx) => (
                  <div key={idx} className="flex justify-between gap-4 text-sm">
                    <div className="flex gap-2">
                       ...
                       <span className="text-[var(--color-brand-dark-blue)] font-medium line-clamp-1">{item.title}</span>
                    </div>
                    <span className="text-[var(--color-brand-brown)]">${(item.price * item.qty).toFixed(2)}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="space-y-3 mb-6 text-sm">
                <div className="flex justify-between text-gray-600">
                  <span>Subtotal</span>
                  <span className="font-medium text-[var(--color-brand-dark-blue)]">${ORDER_SUMMARY.subtotal.toFixed(2)}</span>
                </div>
                <div className="flex justify-between text-gray-600">
                  <span>Shipping</span>
                  <span className="font-medium text-[var(--color-brand-dark-blue)]">${ORDER_SUMMARY.delivery.toFixed(2)}</span>
                </div>
